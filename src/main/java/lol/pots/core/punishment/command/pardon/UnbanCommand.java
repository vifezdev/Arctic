...
public class UnbanCommand {
    ...
    PunishmentType ipBanType = PunishmentType.IP_BAN;

    @Command(name = "", desc = "unbans a player", usage = "<player> <reason> [-s]")
    @Require("arctic.unban")
    public void execute(@Sender CommandSender sender, Profile target, @Text String reason, @Flag('s') boolean silent) {
        ...
        if (target.getActivePunishment(type) == null && target.getActivePunishment(ipBanType) == null) {
            sender.sendMessage(CC.translate("&cThat player isn't " + type.getPardoned().replace("un","") + "."));
        ...
        } else if (target.getActivePunishment(ipBanType) != null) {
            Punishment punishment = target.getActivePunishment(ipBanType);
            PunishmentPardonPacket packet = new PunishmentPardonPacket(target.getUuid(), punishment, pardonedBy, reason, pardonedOn, silent);
            Arctic.getInstance().getRedisHandler().sendPacket(packet);
        }
    }
